...
function setup_vendor() {
    ...
    export BLOOM_ROOT="$3"
    if [ ! -d "$BLOOM_ROOT" ]; then
        echo "\$BLOOM_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$BLOOM_ROOT/$OUTDIR" ]; then
        mkdir -p "$BLOOM_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$BLOOM_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$BLOOM_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$BLOOM_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    printf "# Copyright (C) $YEAR The BloomOS Project\n" > $1
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$BLOOM_ROOT"/vendor/bloom/build/tools/smali/baksmali.jar
        export SMALIJAR="$BLOOM_ROOT"/vendor/bloom/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$BLOOM_ROOT"/vendor/bloom/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$BLOOM_ROOT"/vendor/bloom/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$BLOOM_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$BLOOM_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$BLOOM_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$BLOOM_ROOT"/vendor/bloom/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$BLOOM_ROOT"/"$OUTDIR"/radio
